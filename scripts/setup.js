...
readline.question(`Please provide the FaunaDB admin key\n`, (adminKey) => {
  ...
  streamToPromise(stream)
    ...
    .then((res) => {
      // The GraphQL schema is important, this means that we now have a Anunt Collection and an entries index.
      ...
      return client
        .query(
          q.CreateRole({
            ...
            privileges: [
              {
                resource: q.Collection("Anunt"),
                actions: { read: true, write: true, create: true },
              },
              {
                resource: q.Index("allAnunts"),
                actions: { read: true },
              },
              {
                resource: q.Index("anuntsByTitle"),
                actions: { read: true },
              },
            ],
          })
